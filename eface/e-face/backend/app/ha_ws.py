    ...
    while True:
        try:
            ...
            async with websockets.connect(ws_url, ping_interval=20, ping_timeout=10) as ws:
                ...
                tracked_sub_id = 5
                if tracked_entities:
                    sub = {"id": tracked_sub_id, "type": "subscribe_entities", "entity_ids": tracked_entities}
                    await ws.send(json.dumps(sub))
                    logger.info("Subscribed to %d HA entities", len(tracked_entities))
                else:
                    tracked_sub_id = 1
                    sub = {"id": tracked_sub_id, "type": "subscribe_events", "event_type": "state_changed"}
                    await ws.send(json.dumps(sub))
                ...
                while True:
                    ...
                    if data.get('type') == 'event':
                        ...
                        if data.get('id') == tracked_sub_id or event_type == 'state_changed' or not event_type:
