...
export default {
    ...
    function rebuildEntityMetadata(roomList = []) {
    ...
    function applyEntityUpdate(entityId, stateVal, attributes) {
      ...
      const updateDeviceState = (device) => {
        ...
        if (typeof stateVal !== 'undefined' && stateVal !== null) device.state = stateVal
        if (attributes) {
      ...
      if (mutatedRooms) {
        rooms.value = rooms.value.map(r => ({ ...r, devices: (r.devices || []).map(d => ({ ...d })) }))
      }
      const changed = mutatedRoomDevices || mutatedRooms
      if (!changed) {
